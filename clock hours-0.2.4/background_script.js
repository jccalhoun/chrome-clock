...
var use24HourFormat = false; // New preference for time format
...
// Listen for messages from the options page, popup, or shared settings
browser.runtime.onMessage.addListener((message) => {
    console.log("Received message:", message);

    if (message.colorChanged || message.formatChanged) { // Add check for format changes
        ...
        browser.storage.sync.get({
            ...
            use24HourFormat = storageResult.use24HourFormat; // Update time format
...
function updateClock() {
    ...
    try {
        ...
        for (let currentSize = canvas.height; currentSize >= 1; currentSize--) {
            ...
            let metrics = context.measureText(hours);
            ...
            if (textWidth <= canvas.width - 1 && currentSize <= canvas.height - 1) {
                ...
                console.log(`Found best font size: ${bestFontSize}px for text "${hours}"`);
        ...
        console.log("Final hours font set to:", context.font);

        // Draw the hour text center-aligned with colon
        context.fillText(hours + ":", canvas.width / 2, canvas.height / 2);
        ...
        // Update title with appropriate time format
        ...
        // Schedule the next update using browser.alarms
...
async function initializeExtension() {
    ...
    try {
        ...
        // Only now that we have all the information, update the clock
        console.log("Performing initial clock update with confirmed settings");
        updateClock();
